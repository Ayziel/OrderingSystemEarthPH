    ...
    orders.forEach(order => {
        ...
        order.products.forEach(product => {
            row.innerHTML += `
                ...
                <td>₱ ${product.price.toFixed(2) || 'No price'}</td>
                <td>${order.discount || 0}%</td>
                <td>₱ ${(product.price * product.quantity * (1 - order.discount / 100)).toFixed(2) || 'No amount'}</td>
            `;
...
function openModal(order) {
    ...
    modalContent.innerHTML = `
        ...
        <p><strong>Order Date:</strong> ${order.orderDate
        ...
        <p><strong>Total Amount:</strong> ₱${order.totalAmount ? order.totalAmount.toFixed(2) : 'No amount'}</p>
